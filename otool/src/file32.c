...
static t_ex_ret		get_text_info(t_bin_file *file, struct segment_command *seg)
{
	...
	while (i < seg_nsects)
	{
		...
		if (ft_strcmp(section->sectname, SECT_TEXT) == 0)
		{
			...
			file->text_address = (uint64_t)swap_uint32_if(section->addr,
				file->endian);
			file->text_size = (uint64_t)swap_uint32_if(section->size,
				file->endian);
		}
		i++;
	}
...
static t_ex_ret		init_32(t_bin_file *file)
{
	uint32_t			i;
	struct load_command	*lc;
	uint32_t			header_ncmds;
						...
						enum e_endian endian)
{
	...
	if (init_32(&file) == FAILURE)
		return (FAILURE);
	return (print_output(&file, VALUE_32));
}
